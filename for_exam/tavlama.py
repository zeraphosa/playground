...
def app(alt_sinir,ust_sinir,d,delta, T,Tend,sk):
    ...
    while Tend<T:
        degisim_miktari = get_DegisimMiktari(-(ust_sinir - alt_sinir)*delta/2 ,
                                             (ust_sinir - alt_sinir)*delta/2, d)
        cozum=get_Cozum(d)
        ...
        # amaç fonksiyonunun ne kadar kötüleştiğini ölçer pa: p(deltaE,T)= e üssü (-deltaE/T)
            de=obj_komsu-obj
            pa = np.exp(-de/T)
...
#plt.scatter(array_iterasyon,array_obj)
plt.plot(array_iterasyon, array_obj, color ="red")